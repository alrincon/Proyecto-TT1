...
double* vectorToArray(Matrix& v) {
    double* a = new double[v.getColumnas()];
    for (int i = 1; i <= v.getColumnas(); i++) {
        a[i-1] = v(1,i);
    }
    return a;
}
...
Matrix DEInteg2 ( Matrix (*f) (double, Matrix &), double t, double tout, double relerr, double abserr, int n_eqn, Matrix *y){
    ...
    ode(f, n_eqn, y_array, t, tout, relerr, abserr, iflag, work, iwork);

    Matrix result(1,n_eqn);
    result = arrayToVector(y_array, n_eqn);
    delete [] y_array;
    delete [] work;
    return result;
}