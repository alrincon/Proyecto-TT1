...
void Legendre(int n, int m, double fi, Matrix& pnm, Matrix& dpnm) {
    ...
    for (int i = 1; i < n; i++) {
        pnm(i + 1, i) = sqrt(2 * i + 1.0) * sin(fi) * pnm(i, i);
    }

    for (int i = 1; i < n; i++) {
        dpnm(i + 1, i) = sqrt(2 * i + 1.0) * ((cos(fi) * pnm(i, i)) + (sin(fi) * dpnm(i, i)));
    }
    ...
    while (1) {
        for(int i = k; i < n; i++) {
            pnm(i + 1, j + 1) = sqrt((2 * i + 1.0) / ((i - j) * (i + j))) *((sqrt(2 * i - 1.0) * sin(fi) * pnm(i, j + 1)) - (sqrt(((i + j - 1.0) * (i - j - 1)) / (2 * i - 3)) * pnm(i - 1, j + 1)));
        }
